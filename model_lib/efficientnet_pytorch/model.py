...
class DAXNet(nn.Module):
    """Inspired by EfficientNet model.
    ...
    References:
        ...
        [2] https://arxiv.org/abs/2010.11929 (VisionTransformer)
    """
    ...
    def forward(self, inputs, tax=False):
        ...
        # Test-time Adaption Fusion (TAX)
        if tax:
            x = torch.mean(x, dim=1)
            x_t = torch.mean(x_t, dim=1)
            norm_cnn = torch.norm(x, dim=-1, p=2)
            norm_vit = torch.norm(x_t, dim=-1, p=2)
            w_vit = torch.exp(-norm_cnn) / (torch.exp(-norm_cnn) + torch.exp(-norm_vit))
            w_cnn = torch.exp(-norm_vit) / (torch.exp(-norm_cnn) + torch.exp(-norm_vit))
            w_cnn = torch.cat([w_cnn.unsqueeze(1)] * dim, dim=-1)
            w_vit = torch.cat([w_vit.unsqueeze(1)] * dim, dim=-1)
            x = x * w_cnn + x_t * w_vit
        
        # Summation during training
        else:
            x = x + x_t
...
def jl_efficientnet(task_mode='class', pretrained=True, num_classes=4, **kwargs):
    ...
    """
    func = DAXNet.from_pretrained if pretrained else DAXNet.from_name

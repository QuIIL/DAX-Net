...
class OrdinalSeesawLoss(nn.Module):
    """Implementation of seesaw loss.
    Refers to `Seesaw Loss for Long-Tailed Instance Segmentation (CVPR 2021)
    <https://arxiv.org/abs/2008.10032>`_
    Args:
    ...
    def __init__(self,
                 ...
                 reduction='mean',
                 loss_weight=1.0):
        super(OrdinalSeesawLoss, self).__init__()
        assert not use_sigmoid, '`use_sigmoid` is not supported'
        self.use_sigmoid = False
        self.p = p
        self.q = q
        ...
        self.cls_criterion = o_seesaw_ce_loss
    ...
    def forward(self,
                cls_score, # classification login
                o_cls_score, # regression logit
                labels, # classification label
                o_labels, # regression label
                weight=None,
                avg_factor=None,
                reduction_override=None):
        """Forward function.
        ...
        levels = self.level.repeat(len(o_labels), 1).cuda()
        logit = o_cls_score.repeat(self.num_classes, 1).permute(1, 0)
        logit = torch.abs(logit - levels)
        o_cls_score = -logit
        o_labels = o_labels.long()
        ...
        # calculate loss_cls_classes
        loss_cls = self.loss_weight * self.cls_criterion(cls_score, o_cls_score, labels, weight, self.cum_samples, self.num_classes, self.p, self.q, self.k, self.eps, reduction, avg_factor)
